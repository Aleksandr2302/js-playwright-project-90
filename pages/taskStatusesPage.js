...
export default class TaskStatusesPage {
  constructor(page) {
    this.page = page;
    this.taskStatusesWidget = page.getByRole("menuitem", {
      name: "Task statuses",
    });
    ...
    this.createNewTaskStatusesButton = page.getByRole("link", {
      name: "Create",
    });
  ...
  async taskStatusesVerification(taskStatusesName, taskSlugName) {
    ...
    const taskStatusesNameValue = this.page
      .locator("table tbody tr td:nth-child(3)")
      .filter({ hasText: taskStatusesName });
    const taskStatusesSlugValue = this.page
      .locator("table tbody tr td:nth-child(4)")
      .filter({ hasText: taskSlugName });
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////
  async updateTaskStatusesOnTheWidget(
    taskStatusesOldName,
    taskStatusesNewName,
    taskStatusesSlug,
  ) {
  ...
  async deleteOneTaskStatusesOnTheWidget(taskStatusesName, taskStatusesSlug) {
    ...
    await this.taskStatusesVerificationAfterDeletion(
      taskStatusesName,
      taskStatusesSlug,
    );
  }
  ...
  async deleteBulkTaskStatusesOnWidget() {
    ...
    const selectAllCheckBox = await this.page.getByRole("checkbox", {
      name: "Select all",
    });
    await selectAllCheckBox.check();
    await expect(selectAllCheckBox).toBeChecked();

    const itemSelectedMsg = await this.page.getByRole("heading", {
      name: `${rowCount} items selected`,
      level: 6,
    });
    ...
    await expect(this.warningMessage).toHaveText(
      `${rowCount} elements deleted`,
    );

    const warningText = this.page.locator(".MuiTypography-root", {
      hasText: "Do you want to add one",
    });
